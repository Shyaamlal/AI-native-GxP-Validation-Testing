...
4. **Post-logout UI state (AC-FRS-002.2, .003.1, .003.2).** On the post-sign-out page, inspect the DOM. Verify: no workspace navigation present (search for labels: Dashboard, New Analysis, Analysis History, Users, Usage & Limits, Recording Guide — all zero matches); no user identity area present (search for `testclient@platform.test`, `Test Client 1`, "TC" avatar — all zero matches).
